...
function duplicateEncode(word){
  //split into array, count the letters
  console.log(word)
  const count = word.toLowerCase().split('').reduce( (obj, item) => {
    if(!obj[item]){
      obj[item] = 0 
    }
    obj[item]++
  ...
  //conditional to check if the letters appear more than once and then map original array with ( or ) 
  const newWord = word.toLowerCase().split('').map( x => {
    if(count[x] > 1){
      return x = ')'
    }else{
      return x='('
